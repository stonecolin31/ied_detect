...
# uses the "spidev" package ('pip install spidev')
...
file = "ied_signal.wav"
fft_signal = np.empty(0)

# Create a fake signal to process using the fft for initial code setting purposes
...
f2 = 25000 # 20kHz
f3 = 30000 # 30kHz
# create signal with a 20kHz and 30kHz sinusoid
...
class MCP3201(object):
    ...
    """
    # ** UNCOMMENT THE FOLLOWING FOLLOWING LINES (36-48) TO IMPLEMENT WITH REAL ADC AND SPI FUNCTIONS **
    ...
    def readADC_MSB(self):
        ...
        """
        # ** UNCOMMENT THE FOLLOWING FOLLOWING LINE (57) TO IMPLEMENT WITH REAL ADC AND SPI FUNCTIONS **
        #bytes_received = self._spi.xfer2([0x00, 0x00])
        # ** COMMENT OUT THE FOLLOWING LINE (59) TO IMPLEMENT WITH REAL ADC AND SPI FUNCTIONS **
        bytes_received = signal
        # ** UNCOMMENT THE FOLLOWING FOLLOWING LINE (62-68) TO IMPLEMENT WITH REAL ADC AND SPI FUNCTIONS **
    ...
    def convert_to_voltage(self, adc_output, VREF=3.3):
        ...
        """
        analogue_voltage = adc_output * (int(VREF) / (2 ** 12 - 1))
...
if __name__ == '__main__':
    SPI_bus = 0
    CE = 0
    # ** UNCOMMENT THE FOLLOWING FOLLOWING LINE (161) TO IMPLEMENT WITH REAL ADC AND SPI FUNCTIONS **
    ...
    array = np.empty([numpoints])
    array_size = len(array)
    # time = arange(0, time_length, numpoints)
    time = arange(0, numpoints)
...
# try:
    while i <= array_size:
        ...
        i = i + 1
        print(array)
    FFT.write_data(array)
    fft_signal = FFT.fft_function()
